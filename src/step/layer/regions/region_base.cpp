...
void RegionBase::annotateMeldDepositionRateScalingPath(Path& path) const {
    if (m_sb == nullptr) return;

    if (m_sb->setting<GcodeSyntax>(PRS::MachineSetup::kSyntax) != GcodeSyntax::kMeld) return;

    if (m_sb->setting<MachineType>(PRS::MachineSetup::kMachineType) != MachineType::kFrictionStir) return;

    if (!m_sb->setting<bool>(PRS::MachineSpeed::kMeldDepositionRateScaling)) return;

    QVector<QSharedPointer<SegmentBase>> print_segments;
    QVector<Point> print_points;
    for (const QSharedPointer<SegmentBase>& segment : path.getSegments()) {
        if (segment == nullptr || !segment->isPrintingSegment()) continue;
    ...
    if (print_segments.isEmpty() || print_points.size() < 2) return;
    ...
    for (const QSharedPointer<SegmentBase>& segment : print_segments) {
        ...
        if (segment_settings == nullptr) continue;
