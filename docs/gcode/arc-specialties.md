...
When `G161` absolute-center mode is active, the parser converts absolute `I` and `J` values into relative offsets from the current X/Y position before delegating arcs to `CommonParser`. `G162` and `G164` disable that conversion. Comments containing `RADIAL` or `HELICAL`, excluding travel comments, temporarily force print-state handling so cylindrical preview moves are classified as bead motion. Cylindrical print comments may also include `AXIS_X` and `AXIS_Y` fields so true-width previews and as-printed export can orient the bead cross-section radially, with the flat side toward the cylinder axis.
