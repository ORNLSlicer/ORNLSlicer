...
Cylindrical slicing generates direct toolpaths around a selected vertical cylinder axis. `Path Type` selects whether those paths are radial rings/arcs or rising helices. The axis uses each build part's XY centroid by default, but it can also be set to a custom XY coordinate.
...
2. Set `Slicer Type` to `Cylindrical`.
3. Set `Path Type` to `Radial` or `Helical`.
...
7. Set `Cylinder Axis Mode` if the cylinder axis should use a custom XY coordinate instead of the part centroid.
8. Set `Initial Radius` if the first cylinder or helix should begin away from the axis.
9. Set `Boundary Handling` for the selected `Path Type`.
10. For `Helical`, set `Helical Path Length` when long generated helices should be split into shorter paths. Leave it at `0` to keep each clipped helix fragment as one path.
11. Confirm the printer `Syntax` is `Arc Specialties`. Selecting `Cylindrical` defaults to `Arc Specialties` when the current syntax is not cylindrical-capable.
12. Set `Axis A` to the desired positioner tilt and set `Axis C` if the machine coordinate frame needs an angular offset.
13. Slice and inspect the generated G-code preview before running the machine.
...
The first radius is half a `Layer Height` outward from `Initial Radius`, and later radii advance by `Layer Height`. `Default Bead Width` is the rise per full revolution.
...
| --- | --- | --- |
| `Slicer Type` | Profile > Slicing | Select `Cylindrical` to use cylindrical slicing. |
| `Path Type` | Profile > Slicing | Selects `Radial` rings/arcs or `Helical` rising paths. |
...
| `Cylinder Axis Mode` | Profile > Slicing | Selects whether the cylinder axis is centered on each part's centroid or on a custom XY coordinate. |
| `Cylinder Axis X` / `Cylinder Axis Y` | Profile > Slicing | Custom cylinder axis coordinates when `Cylinder Axis Mode` is `Custom XY`. |
| `Initial Radius` | Profile > Slicing | Inner radial boundary before the half-layer offset is applied. |
| `Helical Path Length` | Profile > Slicing | For `Helical`, maximum length of each generated helical path before it is split. |
| `Number of Arcs per Revolution` | Profile > Slicing | Sets how many G2/G3 moves represent one complete revolution when `Supports G2/G3` is enabled. |

Only relevant settings are shown for the selected path type. `Radial` shows `Boundary Handling` with `Clip`, `Keep`, and `Discard`. `Helical` shows `Boundary Handling` with `Clip` and `Clip Z`, plus helical-only controls.
...
Enabling `Supports G2/G3` writes radial and helical print paths as G2/G3 moves divided according to `Number of Arcs per Revolution`. Those moves use equals-form `I=`/`J=` offsets and place the feedrate at the end of the motion fields. When arc support is disabled, print paths remain segmented G1 moves.

The generated header reports cylindrical geometry, path type, boundary handling, travel lift distance, and positioner settings. Print moves are marked with `RADIAL` or `HELICAL`, and the Arc Specialties parser strips orientation fields for XYZ preview visualization.
...
- Candidate paths are sampled for model clipping. Output uses sampled G1 segments when arc support is disabled, or G2/G3 moves controlled by `Number of Arcs per Revolution` when it is enabled.
- Clipping meshes are applied before cylindrical path generation, but slicing vector settings are not used by cylindrical slicing.
...
- The G-code header identifies `Arc Specialties` and the selected `Path Type`.
...
- The cylinder axis is centered where expected for the selected `Cylinder Axis Mode`.
